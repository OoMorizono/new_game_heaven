    ...
    <div class="row">
        ...
        <div class="col-8 rounded">
            <div class="col-12 bg-white mb-3 rounded">
                ...
                <div class="mb-3">
                    <h3 class="text-danger">2,000<span class="h5">コイン</span></h3>
                </div>
                <div class="content pb-3 mt-1">
                    <form action="{{ asset('charge') }}" method="POST">
                        {{ csrf_field() }}
                        <script src="https://checkout.stripe.com/checkout.js" class="stripe-button"
                            data-key="{{ env('STRIPE_KEY') }}" data-amount="2000" data-name="Stripe Demo"
    ...
    <div class="container">
        <div class="row">
            <button class="" style="background-color:#efefef;">
                <a href="{{ route('posts.edit', $post) }}" class="" style="color:#8c94aa;">編集</a>
            </button>
            <form action="{{ route('posts.destroy', $post) }}" method="post">
                @csrf
                @method('DELETE')
                <input type="submit" value="削除" onclick="if(!confirm('削除しますか？')){return false};" class=""
                    style="background-color:#efefef; color:#8c94aa;">
            </form>
        </div>
    </div>
